//! Geometry engine - tessellation for CoD1 / idTech3 `.map` content.
//!
//! Convex brushes:
//! - Each face polygon is generated by building a large winding on the face plane and clipping it
//!   against all other planes (Quake-style `BaseWindingForPlane` + `ClipWindingEpsilon`).
//! - This is generally more numerically stable than collecting candidate vertices via all
//!   3-plane intersections and filtering with an "on plane" epsilon.
//!
//! Patch surfaces:
//! - `patchTerrainDef3`: triangulate the stored vertex grid, using `turned_edge` to choose the
//!   diagonal for each quad.
//! - `patchDef5`: tessellate quadratic Bezier surfaces using per-segment subdivision.
...
/// Converts a convex brush into renderable per-face polygons (vertex + local index buffers).
///
...
/// Flattens a brush into a single triangle list mesh (positions + indices).
///
...
/// Mutates a brush plane in-place (live editing support)
