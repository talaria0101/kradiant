...
impl View3D {
    fn forward_from_angles(angles: Vec3) -> Vec3 {
        ...
        Vec3::new(yaw.cos() * pitch.cos(), yaw.sin() * pitch.cos(), pitch.sin()).normalize()
    }
    ...
    pub fn draw_impl(
        ...
        .build(|| {
            ...
            draw.with_clip_rect(p, [p[0] + w, p[1] + h], || {
                draw.add_rect(p, [p[0] + w, p[1] + h], imgui_color_to_u32(palette.view2d_bg))
                .filled(true)
                .build();
